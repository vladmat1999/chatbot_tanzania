...
class UserState():
    ...
    def __init__(self,state,substate = 0, data = "", name = ""):
        self.state = state;
...
def getUserState(ID):
    try:
        cursor.execute("SELECT * FROM users_states WHERE Id = %s", (int(ID),))
        data = cursor.fetchone()
        return UserState(data[1],data[2],data[3])
    except IndexError:
        print("IndexError - probabil nu a fost gasit user-ul sau a fost o problema \
                cu fetch-ul")
        return None
...
def setUserState(ID, state, substate = 0, data = ""):
    cursor.execute("update users_states set State = %s, Substate = %s, Data = %s", (state,substate,data))
    conn.commit()

def insertUserState(ID, state, substate = 0, data = ""):
    cursor.execute("insert into users_states values(%s,%s,%s,%s)", (ID,state,substate,data))
    conn.commit()
